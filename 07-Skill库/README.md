...
## 简介
本 Skill 库包含 **68 个专业 Skill**，涵盖软件开发、上下文工程、多智能体架构、文件处理、前端设计、网络搜索、自动化工具、钉钉集成、多模态处理等多个领域。每个 Skill 都是自包含的知识包，为 Claude 提供特定领域的专业指导、工作流程和工具集成。
...
| Skill | 用途 |
|-------|------|
| **multimodal** | 多模态图像识别 - 支持图片内容识别，自动切换 Kimi/Qwen 模型 |
...
Claude：
1. 调用 multimodal Skill
2. 自动选择最优模型（Kimi-K2.5 / Qwen3.5）
3. 返回图片内容识别结果
```
