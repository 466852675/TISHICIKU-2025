...
```bash
# 智能添加（首先发现内容）
python scripts/run.py ask_question.py --question "这个笔记本的内容是什么？涵盖什么主题？简要而简洁地提供完整概述" --notebook-url "[URL]"
# 然后使用发现的信息添加
...
├── library.json       # 笔记本元数据
├── auth_info.json     # 认证状态
└── browser_state/     # 浏览器 cookies
    └── state.json
```

**安全：** 受 `.gitignore` 保护，永远不要提交。
