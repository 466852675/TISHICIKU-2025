---
name: writing-skills
description: 在创建新技能、编辑现有技能或在部署前验证技能时使用
---
...
**个人技能位于代理特定的目录中（Claude Code 为 `~/.claude/skills`，Codex 为 `~/.codex/skills`）**
...
**官方指南：** 有关 Anthropic 的官方技能编写最佳实践，请参阅 anthropic-best-practices.md。本文档提供了与此技能中 TDD 方法相辅相成的额外模式和指南。
...
- 机械约束（如果可以用正则/验证强制执行，则自动化——将文档留给判断调用）
...
**为什么这很重要：** 测试表明，当描述总结技能的工作流程时，Claude 可能会遵循描述而不是阅读完整的技能内容。一个说 "code review between tasks" 的描述导致 Claude 只做一次审查，即使技能的流程图清楚地显示了两个审查（规范合规然后代码质量）。

当描述更改为仅 "Use when executing implementation plans with independent tasks"（无工作流程摘要）时，Claude 正确阅读了流程图并遵循了两阶段审查过程。

**陷阱：** 总结工作流程的描述会创建 Claude 将采取的捷径。技能主体成为 Claude 跳过的文档。

```yaml
# ❌ 不好：总结工作流程 - Claude 可能会遵循此而不是阅读技能
...
# ✅ 好：仅触发条件
description: 在实现任何功能或修复错误之前，在编写实现代码之前使用
```

**内容：**
- 使用具体触发器、症状和表明此技能适用的情况
- 描述 *问题*（竞争条件、不一致行为）而不是 *特定于语言的症状*（setTimeout、sleep）
...
# ✅ 好：以 "在...时使用" 开头，描述问题，无工作流程
description: 在测试存在竞争条件、时间依赖或通过/失败不一致时使用
...
| "如果出现问题，我会测试" | 问题 = 代理无法使用技能。在部署之前测试。 |
| "测试太乏味" | 测试比在生产中调试坏技能更不乏味。 |
| "我确信它很好" | 过度自信保证有问题。反正要测试。 |
| "学术审查就足够了" | 阅读 ≠ 使用。测试应用场景。 |
| "没有时间测试" | 部署未经测试的技能会浪费更多时间以后修复它。 |

**所有这些都意味着：在部署之前测试。没有例外。**

## 使技能防弹合理化

执行纪律的技能（如 TDD）需要抵制合理化。代理很聪明，会在压力下找到漏洞。

**心理学说明：** 理解为什么说服技巧有效有助于你系统地应用它们。有关研究基础（Cialdini, 2021; Meincke et al., 2025）关于权威、承诺、稀缺、社会认同和团结原则，请参阅 persuasion-principles.md。
...
这切断了一整类 "我遵循精神" 的合理化解释。

### 构建合理化表格

从基线测试中捕获合理化解释（参见下面的测试部分）。代理提出的每个借口都进入表格：

```markdown
| 借口 | 现实 |
|--------|---------|
| "太简单而不需要测试" | 简单的代码也会破坏。测试需要 30 秒。 |
| "我之后会测试" | 测试立即通过证明不了什么。 |
| "之后的测试实现相同的目标" | 测试之后 = "这做什么？" 测试之前 = "这应该做什么？" |
```
...
编写解决这些特定合理化解释的技能。不要为假设的情况添加额外内容。

使用技能运行相同的场景。代理现在应该遵守。

### REFACTOR：关闭漏洞

代理找到了新的合理化解释？添加显式计数器。重新测试直到防弹。
...
如果你遵循代码的 TDD，请遵循技能的 TDD。这是应用于文档的相同纪律。
