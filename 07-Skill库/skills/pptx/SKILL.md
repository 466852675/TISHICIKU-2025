---
name: pptx
description: "演示文稿创建、编辑和分析。当 Claude 需要处理演示文稿（.pptx 文件）时：(1) 创建新演示文稿，(2) 修改或编辑内容，(3) 处理布局，(4) 添加注释或演讲者备注，或任何其他演示文稿任务"
...
用户可能会要求你创建、编辑或分析 .pptx 文件的内容。.pptx 文件本质上是一个包含 XML 文件和其他资源的 ZIP 压缩包，你可以读取或编辑。针对不同的任务，你有不同的工具和工作流程可用。
...
### 原始 XML 访问
你需要原始 XML 访问的情况包括：注释、演讲者备注、幻灯片布局、动画、设计元素和复杂格式。对于这些功能，你需要解压演示文稿并读取其原始 XML 内容。

#### 解压文件
`python ooxml/scripts/unpack.py <office_file> <output_dir>`

**注意**：unpack.py 脚本位于相对于项目根目录的 `skills/pptx/ooxml/scripts/unpack.py`。如果此路径下脚本不存在，请使用 `find . -name "unpack.py"` 来定位它。
...
- **勇于冒险**：尝试意想不到的组合 - 医疗演示不一定是绿色，金融不一定是海军蓝
- **构建你的调色板**：选择 3-5 种协调的颜色（主色 + 辅助色 + 强调色）
- **确保对比度**：文本必须在背景上清晰可读

**示例调色板**（用这些激发创意 - 选择一个、改编它，或创建你自己的）：

1. **经典蓝**：深蓝（#1C2833）、石板灰（#2E4053）、银色（#AAB7B8）、米白（#F4F6F6）
2. **青绿与珊瑚**：青绿（#5EA8A7）、深青绿（#277884）、珊瑚色（#FE4447）、白色（#FFFFFF）
3. **大胆红**：红色（#C0392B）、亮红（#E74C3C）、橙色（#F39C12）、黄色（#F1C40F）、绿色（#2ECC71）
4. **暖腮红**：淡紫（#A49393）、腮红（#EED6D3）、玫瑰色（#E8B4B8）、奶油色（#FAF7F2）
...
8. **粉与紫**：粉色（#F8275B）、珊瑚色（#FF574A）、玫瑰色（#FF737D）、紫色（#3D2F68）
9. **酸橙与李子**：酸橙（#C5DE82）、李子色（#7C3A5F）、珊瑚色（#FD8C6E）、蓝灰（#98ACB5）
...
16. **复古大地色**：芥末黄（#E3B448）、鼠尾草（#CBD18F）、森林绿（#3A6B35）、奶油色（#F4F1DE）
17. **海岸玫瑰**：老玫瑰（#AD7670）、海狸色（#B49886）、蛋壳色（#F3ECDC）、灰白（#BFD5BE）
...
### 布局技巧
**创建带有图表或表格的幻灯片时：**
- **双列布局（首选）**：使用横跨全宽的标题，然后在下方分两列 - 一列放文本/项目符号，另一列放特色内容。这提供更好的平衡，使图表/表格更易读。使用 flexbox 配合不等的列宽（例如 40%/60% 分割）来优化每种内容类型的空间。
- **全幻灯片布局**：让特色内容（图表/表格）占据整个幻灯片以获得最大影响和可读性
- **切勿垂直堆叠**：不要将图表/表格放在单列文本下方 - 这会导致可读性和布局问题

### 工作流程
1. **强制 - 阅读整个文件**：从头到尾完整阅读 [`html2pptx.md`](html2pptx.md)。**阅读此文件时切勿设置任何范围限制。** 在继续创建演示文稿之前，阅读完整文件内容以获取详细语法、关键格式规则和最佳实践。
2. 为每张幻灯片创建具有适当尺寸的 HTML 文件（例如 16:9 为 720pt × 405pt）
   - 对所有文本内容使用 `<p>`、`<h1>`-`<h6>`、`<ul>`、`<ol>`
   - 对要添加图表/表格的区域使用 `class="placeholder"`（用灰色背景渲染以便可见）
   - **关键**：首先使用 Sharp 将渐变和图标栅格化为 PNG 图像，然后在 HTML 中引用
   - **布局**：对于带有图表/表格/图片的幻灯片，使用全幻灯片布局或双列布局以获得更好的可读性
...
编辑现有 PowerPoint 演示文稿中的幻灯片时，你需要处理原始 Office Open XML (OOXML) 格式。这涉及解压 .pptx 文件、编辑 XML 内容，然后重新打包。

### 工作流程
1. **强制 - 阅读整个文件**：从头到尾完整阅读 [`ooxml.md`](ooxml.md)（约 500 行）。**阅读此文件时切勿设置任何范围限制。** 在进行任何演示文稿编辑之前，阅读完整文件内容以获取有关 OOXML 结构和编辑工作流程的详细指导。
...
当你需要创建遵循现有模板设计的演示文稿时，你需要在替换占位符内容之前复制并重新排列模板幻灯片。

### 工作流程
1. **提取模板文本并创建视觉缩略图网格**：
   * 提取文本：`python -m markitdown template.pptx > template-content.md`
   * 读取 `template-content.md`：读取整个文件以了解模板演示文稿的内容。**阅读此文件时切勿设置任何范围限制。**
...
3. **基于模板清单创建演示文稿大纲**：
   * 查看步骤 2 中的可用模板。
   * 为第一页幻灯片选择介绍或标题模板。这应该是第一批模板之一。
   * 为其他幻灯片选择安全的、基于文本的布局。
   * **关键：将布局结构与实际内容匹配**：
     - 单列布局：用于统一叙述或单一主题
     - 双列布局：仅当你有恰好 2 个不同的项目/概念时使用
     - 三列布局：仅当你有恰好 3 个不同的项目/概念时使用
     - 图片 + 文本布局：仅当你有实际图片要插入时使用
     - 引用布局：仅用于实际的人物引用（带出处），绝不用于强调
     - 切勿使用比你内容更多的占位符的布局
   ...
   * 保存 `outline.md`，包含内容**和**利用可用设计的模板映射
   * 示例模板映射：
      ```
      # 要使用的模板幻灯片（0 基索引）
      # 警告：验证索引是否在范围内！73 张幻灯片的模板索引为 0-72
...
4. **使用 `rearrange.py` 复制、重新排序和删除幻灯片**：
   * 使用 `scripts/rearrange.py` 脚本按所需顺序创建新演示文稿：
     ```bash
...
5. **使用 `inventory.py` 脚本提取所有文本**：
   * **运行清单提取**：
     ...
     ```
   * **读取 text-inventory.json**：读取整个 text-inventory.json 文件以了解所有形状及其属性。**阅读此文件时切勿设置任何范围限制。**
   ...
   * 关键特性：
     ...
     - **幻灯片编号已过滤**：SLIDE_NUMBER 占位符类型的形状自动从清单中排除
...
6. **生成替换文本并将数据保存到 JSON 文件**
   ...
   - **自动清除**：清单中的**所有**文本形状将被清除，除非你为它们提供 "paragraphs"
   - 对需要内容的形状添加 "paragraphs" 字段（不是 "replacement_paragraphs"）
   - 替换 JSON 中没有 "paragraphs" 的形状将自动清除其文本
   - 生成占位符文本的适当替换内容
   - 使用形状大小确定适当的内容长度
   - **关键**：包含原始清单中的段落属性 - 不要只提供文本
   - **重要**：当 bullet: true 时，**不要**在文本中包含项目符号符号（•、-、*）- 它们会自动添加
   - **基本格式规则**：
...
7. **使用 `replace.py` 脚本应用替换**
   ...
   脚本将：
   - 首先使用 inventory.py 的函数提取**所有**文本形状的清单
...
- 自定义前缀：`python scripts/thumbnail.py template.pptx my-grid`
  - 注意：如果你想在特定目录中输出，输出前缀应包含路径（例如 `workspace/my-grid`）
...
要直观地分析 PowerPoint 幻灯片，使用两步流程将它们转换为图像：
...
2. **将 PDF 页面转换为 JPEG 图像**：
   ...
   ```
   这会创建类似 `slide-1.jpg`、`slide-2.jpg` 等的文件。
