...
**重要：在开始之前请阅读整个文档。** 关键 XML 架构规则和格式化要求贯穿全文。错误的实现可能导致 PowerPoint 无法打开的无效 PPTX 文件。

## 技术指南

### 架构合规性
- **`<p:txBody>` 中的元素顺序**：`<a:bodyPr>`, `<a:lstStyle>`, `<a:p>`
- **空白字符**：为有前导/尾随空格的 `<a:t>` 元素添加 `xml:space='preserve'`
...
6. **清理未使用的媒体**：从 `ppt/media/` 中删除孤立图片
