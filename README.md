...
智能体配置模板是一个标准化、模块化的AI开发环境配置库，提供从框架搭建到生产部署的全套解决方案。通过精心设计的配置模板和专家级提示词，帮助用户**5分钟快速搭建**高质量AI助手开发环境。
...
# 2. 复制基础配置
cp 02-服务配置/claude-mcp-config.json my-config/

# 3. 修改配置
# 编辑 my-config/claude-mcp-config.json
```

#### 添加自定义专家
```bash
# 在对应目录创建新专家
touch 04-提示词专家/04-通用专家/我的专家.md
```
...
#### 新增框架配置
```bash
mkdir 01-框架配置/vue-admin
touch 01-框架配置/vue-admin/v1-system-prompt.md
```

#### 新增提示词专家
```bash
mkdir 04-提示词专家/05-新领域/
touch 04-提示词专家/05-新领域/新专家.md
```
...
1. ⬇️ **克隆项目**: `git clone <repository-url>`
